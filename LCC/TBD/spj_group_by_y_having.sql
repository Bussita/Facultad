...
SELECT *,"Query1" FROM SPJ
GROUP BY `S#`;

/* Decimos que queremos hacer con las otras columnas  */
SELECT `S#`, COUNT(DISTINCT(`P#`)), COUNT(DISTINCT(`J#`)), SUM(CANTIDAD), "Query 2" FROM SPJ
GROUP BY `S#`;

/* Solo agrupa los proveedores cuya cantidad de envios supere los 700 */
SELECT `S#`, SUM(CANTIDAD), "Query 3" FROM SPJ
WHERE CANTIDAD >= 700
GROUP BY `S#`;
...
*/
SELECT `S#`, SUM(CANTIDAD), "Query 4" FROM SPJ
GROUP BY `S#`
HAVING CANTIDAD >= 700;

/* Agrupa los proveedores y filtra los que sumenun total de envios mayor o igual a 700*/
SELECT `S#`, SUM(CANTIDAD), "Query 5" FROM SPJ
GROUP BY `S#`
HAVING SUM(CANTIDAD) >= 700;

/* Agrupa los proveedores y filtra los que sumenun total de envios mayor o igual a 700*/
SELECT `S#`, SUM(CANTIDAD) AS `ENVIOS TOTALES`, "Query 6" FROM SPJ
GROUP BY `S#`
HAVING `ENVIOS TOTALES` >= 700;

/* Toma el primer valor de S# */
SELECT `S#`, SUM(CANTIDAD), "Query 7" FROM SPJ;
...
SELECT `S#`, SUM(CANTIDAD), "Query 8" FROM SPJ
...
SELECT `S#`, SUM(CANTIDAD), "Query 9" FROM SPJ
GROUP BY `S#`
