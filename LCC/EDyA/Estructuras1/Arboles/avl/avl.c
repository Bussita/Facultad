...
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato, FuncionComparadora comp, FuncionDestructora destroy){
  if(raiz == NULL){
    return raiz;
    ...
    if(raiz->der == NULL && raiz->izq == NULL){
      ...
      free(raiz);
      return NULL;
    }else if((raiz->der != NULL && raiz->izq==NULL)||(raiz->der==NULL && raiz->izq!=NULL)){
      AVL_Nodo* reenlace = raiz->der ? raiz->der : raiz->izq;
      destroy(raiz->dato);
      free(raiz);
      return reenlace;
    }else{
      void* valor_reemplazo = bstree_nodo_menor_dato(raiz->der);
      dest(raiz->dato);
