...
/**
 * avl_nodo_balancear: Dado un nodo de un arbol AVL, lo balancea considerando
 * que su |factor_balanceo| <= 2, y que los dos subarboles estan balanceados
*/
static AVL_Nodo* avl_nodo_balancear(AVL_Nodo* raiz) {
...
static AVL_Nodo* avl_nodo_insertar(AVL_Nodo* raiz, void* dato,
  ...
  if (raiz == NULL) // insertamos el nuevo elemento
    ...
    return avl_nodo_balancear(raiz);
  }
  ...
  else return raiz; // no agregar elementos repetidos
}
...
static void* bstree_nodo_menor(AVL_Nodo* raiz) {
...
/**
 * avl_eliminar: Elimina un dato si se encuentra en el arbol, manteniendo
 * la propiead de los arboles AVL.
*/
static AVL_Nodo* avl_nodo_eliminar(AVL_Nodo* raiz, void* dato,
  ...
  if (comp(dato, raiz->dato) == 0) { // Nos encontramos en el nodo
    ...
    if (raiz->der == NULL && raiz->izq == NULL) { // Si no tiene hijos
    ...
    else if ((raiz->der != NULL && raiz->izq == NULL) || // Tiene un hijo no nulo
             (raiz->der == NULL && raiz->izq != NULL)) {

      AVL_Nodo* reenlace = raiz->der ? raiz->der : raiz->izq;
      dest(raiz->dato);
      free(raiz);
      return reenlace;
    }
      ...
      raiz->der = avl_nodo_eliminar(raiz->der, valor_reemplazo, comp, skip_destroy);
    ...
    raiz->der = avl_nodo_eliminar(raiz->der, dato, comp, dest); 
    return avl_nodo_balancear(raiz);
    ...
    raiz->izq = avl_nodo_eliminar(raiz->izq, dato, comp, dest);
    return avl_nodo_balancear(raiz);
  }
}
...
int avl_validar(AVL arbol) {
  ...
  // Verificamos que sea ABB y cumpla la propiedad de AVL
