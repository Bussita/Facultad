...
int main() {
  ...
  AVL arbol = avl_crear(comparar_puntero_entero,copiar_puntero_entero,
  ...
  AVL arbol2 = avl_crear(comparar_puntero_entero, copiar_puntero_entero,
    destruir_puntero_entero);
  int numeros[] = { 10, 20, 15, 25, 30, 16, 18, 19 };
  for (int i = 0; i < 8; ++i) {
    avl_insertar(arbol2, numeros + i);
    printf("Se inserto %d. Recorrido inorden: ", numeros[i]);
    avl_recorrer(arbol2, INORDER, imprimir_puntero_entero);
    puts("");
  }
