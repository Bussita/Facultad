...
deleteBST _ E = E
deleteBST x t@(Node l y r)
                    | x < y = Node (deleteBST x l) y r
                    | x > y = Node l y (deleteBST x r)
                    |otherwise = deleteAux t

deleteAux :: Ord a => BST a -> BST a
deleteAux (Node E _ E) = E
deleteAux (Node l _ E) = l
deleteAux (Node E _ r) = r
deleteAux (Node l _ r) = let m = minimo r in Node l m (deleteBST m r)
